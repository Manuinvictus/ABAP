CLASS zbp_generate_bookings_mcs DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_mcs IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_mcs.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_mcs.
*   insert the new table entries
    INSERT ztbooking_mcs FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_mcs INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
